...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dim", default=100, type=int, help="dim for linear bandit, sanity only"
    )
    parser.add_argument(
        "--arm", default=10, type=int, help="arm for linear bandit, sanity only"
    )
    parser.add_argument(
        "--noise", default=1, type=float, help="noise for linear bandit, sanity only"
    )
    ...
    parser.add_argument(
        ...
        default="mushroom",
        metavar="DATASET",
        help="encoding = onehot and multi only",
    )
    parser.add_argument(
        "--shuffle",
        type=bool,
        default=1,
        metavar="1 / 0",
        help="shuffle the data set or not, encoding = onehot and multi only",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=0,
        help="random seed for shuffle, 0 for None, encoding = onehot and multi only",
    )
    ...
    parser.add_argument(
        ...
        default=0.9,
        metavar="r",
        help="ratio for feature norm, encoding = onehot only",
    )

    parser.add_argument(
        "--learner",
        default="linear",
        metavar="linear|neural|kernel|boost",
        help="TS learner",
    )
    parser.add_argument(
        "--inv", default="diag", metavar="diag|full", help="inverse matrix method"
    )
    parser.add_argument("--style", default="ts", metavar="ts|ucb", help="TS or UCB")

    parser.add_argument(
        "--nu", type=float, default=1, metavar="v", help="nu for control variance"
    )
    parser.add_argument(
        "--lamdba",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="network hidden size, learner = neural and diag only",
    )
    ...
    if args.encoding == "sanity":
        ...
        bandit_info = "sanity_{}_{}_{}_{}".format(
            args.dim, args.noise, args.arm, args.size
        )
    elif args.encoding == "onehot":
        b = Bandit_onehot(
            args.dataset, is_shuffle=args.shuffle, seed=use_seed, r=args.r
        )
    ...
    if args.learner == "linear":
        ...
        if args.inv == "diag":
            ...
            RuntimeError("Inverse method not exist")
        ts_info = "{}_kernel_{:.3e}_{:.3e}_{}".format(
            args.style, args.lamdba, args.nu, args.inv
        )
